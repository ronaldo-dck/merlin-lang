...
Semantico::Semantico(std::string outputFileName) {
    ...
    code = {
        ...
        {2,LAMBDAFUNC -> void {}},
        // {3, {gtoken_declare_sq, {gtoken_type, token_id, token_endline, gtoken_declare_sq}}},
        {3,LAMBDAFUNC  -> void {int index = std::stoi(tokens[0].data); for (int i = index - 1; i >= 0; i--) output << tokens[1].data << "_" << i << ": " << get_type_TAC(tokens[0].type) << "= 0" << std::endl;}},
        // {4, {gtoken_declare_sq, {gtoken_type, token_id, token_atr, gtoken_value, token_endline, gtoken_declare_sq}}},
        {4,LAMBDAFUNC  -> void {tokens[0].type == token_string ? output << tokens[1].data + "_0" << ':' << get_type_TAC(tokens[0].type) << "=" << "\"" << tokens[3].data << "\"" << std::endl : output << tokens[1].data + "_0" << ':' << get_type_TAC(tokens[0].type) << "=" << tokens[3].data << std::endl;}},
        // {5, {gtoken_cmd_sq, {gtoken_operation, token_endline, gtoken_cmd_sq}}},
        {5,LAMBDAFUNC  -> void {}},
        // {6, {gtoken_operation, {gtoken_assign_sq}}},
        {6,LAMBDAFUNC  -> void {}},
        ...
        {54,LAMBDAFUNC -> void {}},
        // {55, {gtoken_declare_sq, {gtoken_type, token_id, token_endline}}},
        { 55,LAMBDAFUNC -> void {int index = std::stoi(tokens[0].data); for (int i = index - 1; i >= 0; i--) output << tokens[1].data << "_" << i << ": " << get_type_TAC(tokens[0].type) << "= 0" << std::endl;} },
        // {56, {gtoken_declare_sq, {gtoken_type, token_id, token_atr, gtoken_value, token_endline}}},
        {56,LAMBDAFUNC -> void {tokens[0].type == token_string ? output << tokens[1].data + "_0" << ':' << get_type_TAC(tokens[0].type) << "=" << "\"" << tokens[3].data << "\"" << std::endl : output << tokens[1].data + "_0" << ':' << get_type_TAC(tokens[0].type) << "=" << tokens[3].data << std::endl;}},
        // {57, {gtoken_id, {token_id, token_col_esq, token_id, token_col_dir}}},
